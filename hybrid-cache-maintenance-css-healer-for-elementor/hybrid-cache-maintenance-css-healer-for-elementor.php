 ...
 * Plugin URI:        https://watermelonwebworks.com/hybrid-cache-maintenance-for-elementor-fix-missing-css/
 ...
 * Text Domain:       hybrid-cache-maintenance-css-healer-for-elementor
...
function wm_ech_missing_elementor_notice() {
    ...
    $message = __( 'Hybrid Cache Maintenance & CSS Healer requires Elementor to be installed and active.', 'hybrid-cache-maintenance-css-healer-for-elementor' );
...
function wm_ech_add_cron_schedule( $schedules ) {
    if ( ! isset( $schedules[ WM_ECH_CRON_SCHEDULE ] ) ) {
        $schedules[ WM_ECH_CRON_SCHEDULE ] = array(
            ...
            'display'  => esc_html__( 'Every 12 Hours', 'hybrid-cache-maintenance-css-healer-for-elementor' ),
        );
    }
    return $schedules;
}
